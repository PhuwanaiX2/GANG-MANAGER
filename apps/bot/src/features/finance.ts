...
function getFinanceRequestLabel(type?: string | null, description?: string | null) {
    if (type === 'LOAN') return 'ขอเบิก/ยืมเงิน';
    if (type === 'REPAYMENT') return LOAN_REPAYMENT_LABEL;
    if (type === 'DEPOSIT') return COLLECTION_PAYMENT_LABEL;
    if (type === 'GANG_FEE') return 'ยอดเก็บเงินแก๊ง';
    return description || 'รายการการเงิน';
}

async function notifyFinanceRequester(
    client: Client,
    transaction: any,
    ...
    try {
        ...
        if (!requester?.discordId) return;

        const discordUser = await (client as any).users?.fetch?.(requester.discordId);
        if (!discordUser?.send) return;

        const approved = status === 'APPROVED';
        const embed = new EmbedBuilder()
            .setColor(approved ? 0x57F287 : 0xED4245)
            .setTitle(approved ? '✅ รายการการเงินอนุมัติแล้ว' : '❌ รายการการเงินถูกปฏิเสธ')
            .setDescription(
                approved
                    ? 'ระบบอัปเดตยอดให้เรียบร้อยแล้ว สามารถตรวจยอดล่าสุดได้จากปุ่มสถานะการเงินหรือหน้าเว็บ'
                    : 'รายการนี้ถูกปิดแล้ว หากคิดว่าเกิดข้อผิดพลาดให้ติดต่อหัวหน้าแก๊งหรือเหรัญญิก'
            )
            .addFields(
                { name: 'รายการ', value: getFinanceRequestLabel(transaction.type, transaction.description), inline: false },
                { name: 'จำนวน', value: `฿${Number(transaction.amount || 0).toLocaleString()}`, inline: true },
                { name: approved ? 'อนุมัติโดย' : 'ปฏิเสธโดย', value: actorName || 'แอดมิน', inline: true },
            )
            .setFooter({ text: thaiTimestamp() });

        await discordUser.send({ embeds: [embed] });
    } catch (err) {
        logWarn('bot.finance.requester_dm_failed', {
...
registerButtonHandler('fn_approve_', async (interaction: ButtonInteraction) => {
    ...
    try {
        ...
        await notifyFinanceRequester(interaction.client, transaction, 'APPROVED', member.name || interaction.user.username);
...
registerButtonHandler('fn_reject_', async (interaction: ButtonInteraction) => {
    ...
    try {
        ...
        await notifyFinanceRequester(interaction.client, existing, 'REJECTED', approver.name || interaction.user.username);
