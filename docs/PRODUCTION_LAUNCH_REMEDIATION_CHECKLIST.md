...
|---|---|---|---|---|---|
| 2026-05-18 | Product/UX Redesign | Applied Friendly Ops Layout pass 1: softened shared visual tokens, removed harsh gradient/uppercase console styling, improved sidebar active state, normalized dashboard/admin table labels, localized key customer-facing copy, and tightened member/attendance/leave/finance shells for a friendlier Discord-native SaaS operations feel | `npm run lint -w apps/web`; `npm run build -w apps/web`; `npm run test -w apps/web -- src/tests/ui`; local Playwright visual smoke against dashboard list, overview, members, attendance, leaves, finance, billing, settings, and mobile attendance | Passed; known non-blocking stale Discord CDN avatar returns 404 in local seed data only | Codex |
