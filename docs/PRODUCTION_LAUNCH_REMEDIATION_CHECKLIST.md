...
|---|---|---|---|---|---|
| 2026-05-18 | Product/UX Redesign | Applied Friendly Ops hardening pass 2: added safe avatar proxy/fallback, moved member/profile/finance/leave/attendance/sidebar avatar rendering to a shared component, tightened shared dashboard density/letter spacing, and fixed closed mobile drawer pointer-event interception | `npm run lint -w apps/web`; `npm run build -w apps/web`; `npm run test -w apps/web -- src/tests/ui`; `npm run encoding:verify`; `git diff --check`; local avatar API fallback smoke; Playwright manual attendance mobile smoke; Playwright browser smoke across 11 main customer routes on desktop and mobile | Passed; local Discord self-check-in smoke is blocked by current local Discord API token returning `401 Unauthorized`, not by the UI redesign | Codex |
| 2026-05-18 | Product/UX Redesign | Applied Friendly Ops Layout pass 1: softened shared visual tokens, removed harsh gradient/uppercase console styling, improved sidebar active state, normalized dashboard/admin table labels, localized key customer-facing copy, and tightened member/attendance/leave/finance shells for a friendlier Discord-native SaaS operations feel | `npm run lint -w apps/web`; `npm run build -w apps/web`; `npm run test -w apps/web -- src/tests/ui`; local Playwright visual smoke against dashboard list, overview, members, attendance, leaves, finance, billing, settings, and mobile attendance | Passed; stale Discord CDN avatar issue was found here and resolved in Product/UX hardening pass 2 | Codex |
| 2026-05-17 | Monitoring/Alerts | Added protected runtime alert-test endpoints for Vercel web and Render bot, gated by `ALERT_TEST_TOKEN` or fallback `ALERT_WEBHOOK_URL`, so alert ENV can be verified without causing real production failures | `npm run test -w apps/web -- src/tests/api/alert-test.test.ts src/tests/lib/logger.test.ts`; `npm run test -w apps/bot -- tests/alertTestEndpoint.test.ts tests/logger.test.ts`; `npm run lint -w apps/web`; `npm run build -w apps/web`; `npm run build -w apps/bot` | Passed locally; deploy then run live runtime smoke against web and bot alert-test endpoints | Codex |
| 2026-05-17 | Monitoring/Alerts | Added native Discord webhook adapter for web logger, bot logger, and production monitor alerts while preserving generic JSON webhook support and bearer-token auth for non-Discord receivers | `npm run test -w apps/web -- src/tests/lib/logger.test.ts`; `npm run test -w apps/bot -- tests/logger.test.ts`; `node --check scripts/monitor-production.mjs`; local monitor Discord webhook smoke with `/api/health`, `/health`, `/ready`, and `/webhook` | Passed locally; production ENV still needs real Discord webhook URL on Vercel and Render | Codex |
| 2026-05-17 | Dependency/Tooling | Migrated web lint from deprecated `next lint` to ESLint 9 flat config, aligned Node typings with Node 24, and upgraded `@libsql/client` to remove deprecated install chain from `eslint`, `glob/rimraf/inflight`, and `node-domexception` | `npm run lint -w apps/web`; `npm run audit:dependencies`; `npm run db:generate`; `npm run db:audit:migrations`; `npm test`; `npm run build`; `npm ci` | Passed; only remaining install warnings are upstream `drizzle-kit@0.31.10 -> @esbuild-kit/*`; `drizzle-kit@1.0.0-rc.2` was tested and rejected because migration generation failed with stable `drizzle-orm` | Codex |
| 2026-05-17 | CI/Release Gate | Updated GitHub Actions release gate from Node 20 runtime actions to Node 24 with `actions/checkout@v6` and `actions/setup-node@v6` | `git ls-remote` verified both v6 tags exist; `npm run audit:dependencies`; `npm run validate:env:prod`; `npm run lint -w apps/web`; `npm test`; `npm run build`; GitHub Release Gate run `25988731061` | Passed; Node 20 action runtime warning removed, remaining logs are dependency deprecation warnings only | Codex |
| 2026-05-17 | Paid Billing/Payment UX | Enforced one active payment bill per gang, auto-closes stale duplicate bills, rejects duplicate/invalid SlipOK payment failures as terminal rejected records, preserves rejection evidence, and replaced raw payment status labels with Thai operational copy | `npm run test -w apps/web -- src/tests/api/admin-subscription-payments.test.ts src/tests/api/subscription-payment-requests.test.ts`; `npm run build -w apps/web`; `npm run build -w apps/bot`; `npm run encoding:verify`; `git diff --check` | Passed locally; live PromptPay/SlipOK retest required after deploy | Codex |
...
- [x] Full mobile visual QA ทุกหน้าหลัก
  - Evidence: Playwright browser smoke across 11 main customer routes at 390x844, screenshots saved in `apps/web/.playwright/ui-hard-pass`.
- [x] Full desktop visual QA ทุกหน้าหลัก
  - Evidence: Playwright browser smoke across 11 main customer routes at 1440x900, screenshots saved in `apps/web/.playwright/ui-hard-pass`.
